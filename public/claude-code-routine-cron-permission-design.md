...
- Amazon仕入れの値下げ監視、副業リードの一次仕分け（日次＋営業時間帯限定の時次）、記事下書き生成という、用途の異なる4本のバッチをClaude Code Remoteの**Routine**（スケジュールトリガー機能）に移行した
...
- 本記事の下書き自体、上記のRoutineの1本（作成からおよそ3時間後の初回実行）が生成している一次情報
...
会社名・案件の中身などの機微な情報は伏せ、頻度と権限設計だけを抽出すると以下の通りです。

| 用途 | 頻度（UTC cron） | JST換算 | 主な許可ツール | 書き込み範囲 |
|---|---|---|---|---|
| Amazon仕入れの値下げ通知集計 | `0 23 * * *` | 毎日8:00 | Gmail検索・Notion検索/作成/更新 | Notion DBへの登録・更新のみ |
| 副業リードの一次仕分け（日次） | `0 23 * * *` | 毎日8:00 | Gmail検索/スレッド操作・Notion検索/作成/更新 | NotionDB登録＋Gmailラベル操作 |
| 副業リードの一次仕分け（営業時間帯） | `0 0-12 * * *` | 毎日9:00〜21:00に毎時 | 同上 | 同上 |
| 記事下書き生成（本記事） | `0 17 * * *` | 翌2:05前後 | Bash/Read/Write/Edit/Glob/Grep/WebFetch・Gmail検索・Slack | リポジトリへのbranch push（PR作成不可） |
...
- **パターン1（日次・深夜）**：「昨日1日分をまとめて処理すればいい」性質のバッチ。値下げ通知のように、多少の処理遅延が実害にならないものに向く
- **パターン2（時次・営業時間帯限定）**：新規の問い合わせ・リードのように「早く拾えるほど価値が高い」ものに向く。24時間毎時にすると深夜のAPIコールが無駄になるため、cron側で `0 0-12 * * *` のように稼働レンジを絞る

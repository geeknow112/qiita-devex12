...
A. リスクの大きさは「モデルが指示に従わない確率」と「プロンプトインジェクションのような外部入力で誤動作させられる確率」に依存するため、この記事だけでは定量化できません（自分の運用で実際にインシデントが発生した記録はなく、今回はあくまで攻撃対象領域の広さを実測した記事です）。ただし「技術的に実行可能な操作の集合」が「タスクに必要な操作の集合」より大きいこと自体は、攻撃対象領域（アタックサーフェス）が不必要に広い状態であり、最小権限の原則には反します。
...
本記事のようなAIエージェント運用の権限設計・自動化ノウハウを、Udemyコースとして体系的にまとめています。
